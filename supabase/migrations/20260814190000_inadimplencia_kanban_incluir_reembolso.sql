-- SOMENTE o card/lista do kanban (clients_inadimplencia_list).
-- Inclui títulos vencidos de REEMBOLSO DE DESPESAS no valor em aberto.
-- Não altera cota, Receita, periodo_net_clientes nem plano_contas_na_cota.
...
grupo_saldo_vencido AS (
  SELECT
    ...
    MIN(k.data_vencimento) FILTER (WHERE k.saldo > 0) AS oldest_overdue
  FROM itens_kanban k
  GROUP BY 1
...
COMMENT ON VIEW public.clients_inadimplencia_list IS
  'Lista/kanban inadimplência: saldo vencido do grupo (cota + reembolso de despesas). Não altera a cota da Receita.';
