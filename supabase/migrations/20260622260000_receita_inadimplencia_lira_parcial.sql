-- Combina saldo por item (antecipado / pago após o mês) com diferença faturado − recebido no mês (Lira parcial).
...
COMMENT ON FUNCTION public.receita_inadimplencia_cliente_mes(integer, integer) IS
  'Inad por cliente: min(saldo itens no mês, faturado − recebido no mês). Antecipado zera itens; recebido parcial no mês reduz (ex. Metalcasty).';
