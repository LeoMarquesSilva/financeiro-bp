-- Card Resultado R$ (ao vivo): se o título do período foi pago depois, dá baixa.
-- Evolução / grupo_mes: continuam no snapshot do mês (sem baixa posterior).
-- Estende periodo_net_clientes; grupo_mes chama com p_baixa_posterior = false.

DROP FUNCTION IF EXISTS public.receita_inadimplencia_periodo_net_clientes(integer, integer, integer, boolean);

CREATE FUNCTION public.receita_inadimplencia_periodo_net_clientes(
  p_ano integer,
...
COMMENT ON FUNCTION public.receita_inadimplencia_periodo_net_clientes(integer, integer, integer, boolean, boolean) IS
  'Saldo líquido do período. Faturado: só vencidos. Recebido: caixa do intervalo + (se p_baixa_posterior) pagamentos posteriores dos títulos do período.';

GRANT EXECUTE ON FUNCTION public.receita_inadimplencia_periodo_net_clientes(integer, integer, integer, boolean, boolean) TO anon, authenticated;
...
AS $$
  WITH net AS (
    ...
    FROM public.receita_inadimplencia_periodo_net_clientes(
      p_ano, p_mes, p_mes, p_incluir_inativos, false
    ) n
  )
  SELECT
...
COMMENT ON FUNCTION public.receita_inadimplencia_grupo_mes(integer, integer, boolean) IS
  'Snapshot do mês (sem baixa posterior). Usado na evolução / sheet mensal; meses congelados exibem o fechamento.';
