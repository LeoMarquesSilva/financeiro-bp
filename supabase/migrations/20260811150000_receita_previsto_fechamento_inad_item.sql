-- Inad. do mês no fechamento previsto: faturado não pago item a item (sem saldo consolidado de grupo).
...
BEGIN
  ...
  v_inad_kpi := GREATEST(0, v_em_aberto + v_quitado_pago_depois);
...
COMMENT ON FUNCTION public.receita_previsto_fechamento_mes(integer, integer) IS
  'Previsto = quitado no mês + antecipado + pago depois + em aberto. Inad. KPI = em aberto + pago depois (item a item, sem compensação de grupo).';
