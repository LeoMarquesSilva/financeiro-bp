-- OPEX: extrapolação de despesas fixas por grupo (média YTD do próprio grupo),
-- em vez de repetir a média global em cada grupo fixo.

CREATE OR REPLACE FUNCTION public.opex_extrapolacao_fixas_grupo(
  p_realizado_ytd numeric,
  p_mes_atual integer
)
RETURNS numeric
LANGUAGE sql
...
AS $$
  SELECT CASE
    WHEN p_mes_atual <= 0 OR p_mes_atual >= 12 OR coalesce(p_realizado_ytd, 0) <= 0 THEN 0::numeric
    ELSE round((p_realizado_ytd / p_mes_atual) * (12 - p_mes_atual), 2)
  END;
$$;

COMMENT ON FUNCTION public.opex_extrapolacao_fixas_grupo(numeric, integer) IS
  'Extrapola despesas fixas do grupo: (realizado YTD / meses decorridos) × meses restantes.';

GRANT EXECUTE ON FUNCTION public.opex_extrapolacao_fixas_grupo(numeric, integer) TO anon, authenticated;
