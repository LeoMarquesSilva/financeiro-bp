-- valor_em_atraso_ativos: exclui Cliente inativo e empresas sem grupo_cliente.
...
fin_sum AS (
  SELECT
    ...
    COALESCE(
      sum(f_1.valor_em_atraso) FILTER (
        WHERE COALESCE(p.categoria, '') <> 'Cliente inativo'
          AND COALESCE(NULLIF(TRIM(BOTH FROM eepg.grupo_cliente), ''), '') <> ''
      ),
      0::numeric
    )::numeric(12, 2) AS valor_em_atraso_ativos
  FROM escritorio_empresas_por_grupo eepg
    JOIN relatorio_financeiro_resumo_por_cliente f_1 ON f_1.pessoa_id = eepg.id
    JOIN pessoas p ON p.id = eepg.id
  GROUP BY 1
)
SELECT
  e.grupo_cliente,
  ...
  COALESCE(f.valor_em_atraso_ativos, 0::numeric)::numeric(12, 2) AS valor_em_atraso_ativos
FROM emp_count e
