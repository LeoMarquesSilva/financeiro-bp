-- Valor mensal: soma todas as parcelas em aberto do mês de referência
-- (mês atual se houver parcela nele; senão, o próximo mês com parcela).
-- Antes: DISTINCT ON pegava só a primeira parcela a vencer.
...
COMMENT ON VIEW public.clients_inadimplencia_list IS
  'Painel Inadimplência: saldo do grupo = honorários vencidos (cota) + reembolso de despesas em aberto. Valor mensal = soma das parcelas em aberto do mês de referência.';
