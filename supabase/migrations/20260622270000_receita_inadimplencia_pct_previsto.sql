-- Expõe previsto mensal na evolução (mesmo denominador do gráfico comparativo de receita).
    ...
    COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'pct', CASE
          ...
          WHEN ec.previsto > 0 THEN ROUND(
            (CASE WHEN ec.congelado THEN ec.valor_congelado ELSE ec.valor_calc END / ec.previsto) * 100,
            2
          )
          ELSE 0
        END,
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard inadimplência. pct = inadimplência / previsto mensal (receita_totais_mensais).';
