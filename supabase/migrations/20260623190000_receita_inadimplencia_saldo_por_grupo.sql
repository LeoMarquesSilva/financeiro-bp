-- Inadimplência sempre por grupo_cliente: saldo do grupo = Σ faturado − Σ recebido das empresas.
-- Evita dupla contagem quando vencimento e pagamento caem em razões sociais diferentes (ex.: CDA / Alumínio).
...
COMMENT ON FUNCTION public.receita_inadimplencia_grupo_mes(integer, integer) IS
  'Inadimplência mensal por grupo_cliente: max(0, Σ faturado − Σ recebido das empresas do grupo).';
...
COMMENT ON FUNCTION public.receita_inadimplencia_grupos_periodo(integer, integer, integer) IS
  'Inadimplência do período por grupo_cliente: max(0, Σ faturado − Σ recebido das empresas do grupo).';
  ...
  inad_por_mes AS (
    SELECT
      ...
      (
        ...
        FROM public.receita_inadimplencia_grupo_mes(p_ano, m.mes) g
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard: KPI e evolução por grupo_cliente (saldo consolidado do grupo).';
