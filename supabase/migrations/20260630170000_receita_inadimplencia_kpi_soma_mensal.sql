-- KPI acumulado = soma dos valores mensais da evolução (mesma regra VIOS + antecipado),
-- não saldo líquido agregado do período (que ignorava pagamentos antecipados).
...
AS $$
  ...
  por_mes AS (
    SELECT
      ...
      g.qtd_clientes_inad
    FROM meses m
    CROSS JOIN LATERAL public.receita_inadimplencia_grupo_mes(p_ano, m.mes, p_incluir_inativos) g
    WHERE g.inadimplencia > 0
  )
  SELECT
    pm.grupo_cliente,
    ROUND(SUM(pm.inadimplencia), 2)::numeric(15, 2) AS valor,
    COUNT(DISTINCT pm.mes)::integer AS qtd_meses,
    MAX(pm.qtd_clientes_inad)::integer AS qtd_clientes
  FROM por_mes pm
  GROUP BY pm.grupo_cliente
  HAVING SUM(pm.inadimplencia) > 0
  ORDER BY valor DESC, pm.grupo_cliente;
$$;

COMMENT ON FUNCTION public.receita_inadimplencia_grupos_periodo(integer, integer, integer, boolean) IS
  'Inadimplência do período por grupo: soma mensal max(0, faturado − recebido), alinhada à evolução.';
...
BEGIN
  ...
  evolucao_calc AS (
    SELECT
      ...
      CASE
        WHEN f.ano IS NOT NULL THEN f.valor_total
        ELSE COALESCE(i.inad, 0)::numeric(15, 2)
  ...
  SELECT
    ...
    COALESCE(ROUND(SUM(ec.valor_exibido), 2), 0)::numeric(15, 2)
  INTO v_evolucao, v_previsto_periodo, v_valor_periodo
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard inadimplência: KPI acumulado = soma dos valores mensais da evolução (VIOS + antecipado).';
