-- Acumulado e evolução usam sempre valor calculado (VIOS atual), não snapshot congelado.
  ...
  SELECT
    COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'valor_congelado', ec.valor_congelado
      ) ORDER BY ec.mes
    ), '[]'::jsonb),
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard inadimplência. Valores sempre calculados (cliente_mes); congelado indica snapshot histórico.';
