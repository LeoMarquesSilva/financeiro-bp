-- Inadimplência do período agregada por grupo_cliente (mesma regra Lira mensal).
...
AS $$
  ...
  SELECT
    cm.grupo_cliente,
    ROUND(SUM(cm.inadimplencia), 2)::numeric(15, 2) AS valor,
    COUNT(DISTINCT cm.mes)::integer AS qtd_meses,
    COUNT(DISTINCT cm.cliente)::integer AS qtd_clientes
  FROM cliente_mes cm
  GROUP BY cm.grupo_cliente
  HAVING SUM(cm.inadimplencia) > 0
  ORDER BY valor DESC, cm.grupo_cliente;
$$;

COMMENT ON FUNCTION public.receita_inadimplencia_grupos_periodo(integer, integer, integer) IS
  'Inadimplência do período por grupo_cliente — soma mensal Lira das empresas do grupo.';

GRANT EXECUTE ON FUNCTION public.receita_inadimplencia_grupos_periodo(integer, integer, integer) TO anon, authenticated;

-- Inclui grupo_cliente na lista por empresa (para drill-down dentro do grupo).
...
-- Top 5 e lista passam a usar grupos no dashboard.
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard inadimplência: KPI = soma mensal Lira; top5 = grupos do período.';
