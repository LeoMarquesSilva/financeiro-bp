-- Regra VIOS + ajuste por cliente (ex.: Lira Alimentos):
-- Se recebido no mês >= faturado no mês, o cliente não entra como inadimplente
-- naquele mês, mesmo com títulos antigos em aberto.
-- Inadimplência mensal = SUM por cliente de GREATEST(0, faturado_mês − recebido_mês).
...
AS $$
  WITH fat AS (
    SELECT
      ...
      SUM(COALESCE(v.valor_item, 0))::numeric(15, 2) AS total
  ...
  rec AS (
    SELECT
      COALESCE(NULLIF(trim(v.cliente), ''), 'Sem cliente') AS cliente,
      SUM(COALESCE(v.valor_pago_item, 0))::numeric(15, 2) AS total
    FROM public.receita_itens_inadimplencia_elegiveis v
  ...
  SELECT
    ...
    COALESCE(f.total, 0) AS faturado,
    COALESCE(r.total, 0) AS recebido,
    GREATEST(0, COALESCE(f.total, 0) - COALESCE(r.total, 0))::numeric(15, 2) AS inadimplencia
  FROM fat f
  FULL OUTER JOIN rec r ON r.cliente = f.cliente;
$$;
...
SET search_path = public
AS $$
  SELECT COALESCE(SUM(c.inadimplencia), 0)::numeric(15, 2)
...
COMMENT ON FUNCTION public.receita_inadimplencia_cliente_mes(integer, integer) IS
  'Por cliente: faturado (vencimento no mês) e recebido (pagamento no mês). Inad = max(0, faturado − recebido).';

COMMENT ON FUNCTION public.receita_inadimplencia_mes_faturado(integer, integer) IS
  'Inadimplência do mês: soma por cliente de max(0, faturado − recebido).';
...
GRANT EXECUTE ON FUNCTION public.receita_inadimplencia_cliente_mes(integer, integer) TO anon, authenticated;
