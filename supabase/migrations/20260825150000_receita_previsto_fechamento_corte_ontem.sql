-- Inad. KPI do fechamento do previsto: mesmo corte da inadimplência.
-- Vencimento = hoje ainda não é vencido.
...
DECLARE
  ...
  v_corte date := public.receita_inadimplencia_corte_vencimento(p_ano, p_mes);
BEGIN
  SELECT
  ...
  WHERE (i.tipo IS NULL OR i.tipo = 'RECEBER')
    ...
    AND EXTRACT(YEAR FROM i.data_vencimento)::integer = p_ano
    AND EXTRACT(MONTH FROM i.data_vencimento)::integer = p_mes
    AND i.data_vencimento::date <= v_corte
    AND (
      i.data_pagamento IS NULL
      OR i.data_pagamento > v_mes_fim
    );
...
COMMENT ON FUNCTION public.receita_previsto_fechamento_mes(integer, integer) IS
  'Inad. KPI = vencido até o corte (ontem no mês corrente) e não quitado no mês. Vencimento = hoje não entra.';
