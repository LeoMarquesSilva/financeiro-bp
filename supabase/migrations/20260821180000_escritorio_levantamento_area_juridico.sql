-- Levantamento Escritório: filtro de área alinhado ao painel Eficiência Jurídico.
-- Reestruturação agrupa Insolvência e Cível | Insolvência.
...
CREATE OR REPLACE FUNCTION public.escritorio_levantamento_area_match(
  p_filtro text,
  p_valor text
)
RETURNS boolean
...
AS $$
  SELECT CASE
    WHEN NULLIF(trim(COALESCE(p_filtro, '')), '') IS NULL THEN TRUE
    WHEN NULLIF(trim(COALESCE(p_valor, '')), '') IS NULL THEN FALSE
    WHEN lower(trim(p_filtro)) IN ('reestruturação', 'reestruturacao') THEN
      lower(trim(p_valor)) IN (
        'reestruturação',
        'reestruturacao',
        'insolvência',
        'insolvencia',
    ...
    ELSE lower(trim(p_valor)) = lower(trim(p_filtro))
  END;
$$;

COMMENT ON FUNCTION public.escritorio_levantamento_area_match(text, text) IS
  'Match de área no levantamento Escritório — Reestruturação inclui Insolvência e Cível | Insolvência (padrão indicadores).';
...
-- Racional: mesma regra de área (substitui match literal por escritorio_levantamento_area_match).
...
COMMENT ON FUNCTION public.escritorio_levantamento_resumo_v2(date, date, text[], text) IS
  'KPIs levantamento Escritório. Área: padrão Eficiência Jurídico (Reestruturação inclui Insolvência e Cível | Insolvência).';

COMMENT ON FUNCTION public.escritorio_levantamento_racional_v2(text, date, date, text[], text, text, integer) IS
  'Racional levantamento Escritório — mesma regra de área que resumo_v2.';
