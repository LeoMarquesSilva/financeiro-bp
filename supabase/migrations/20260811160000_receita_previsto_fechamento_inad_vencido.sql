-- Inad. KPI: somente itens com vencimento no mês já vencidos até hoje (ou fim do mês, se passado).
...
DECLARE
  ...
  v_corte date := LEAST(v_mes_fim, CURRENT_DATE);
BEGIN
  SELECT
...
COMMENT ON FUNCTION public.receita_previsto_fechamento_mes(integer, integer) IS
  'Inad. KPI = vencido até hoje no mês e não quitado no mês (sem compensação de grupo).';
