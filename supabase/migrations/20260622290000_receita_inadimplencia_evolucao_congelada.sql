-- Evolução mensal: meses congelados usam snapshot; último mês (*) usa cálculo ao vivo.
-- Acumulado do período: sempre soma dos valores calculados (cliente_mes).
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Evolução: congelado=snapshot; * = calculado ao vivo. Acumulado: soma calculada (cliente_mes).';
