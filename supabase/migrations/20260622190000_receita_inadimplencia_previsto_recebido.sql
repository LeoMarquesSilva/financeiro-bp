-- Inadimplência = previsto (faturado no mês) − recebido (pago no mês, faturado no mesmo mês).
-- Recebimentos de títulos faturados em meses anteriores não entram no recebido do mês.
...
SET search_path = public
AS $$
  SELECT COALESCE(SUM(COALESCE(v.valor_pago_item, 0)), 0)::numeric(15, 2)
  FROM public.receita_itens_inadimplencia_elegiveis v
  WHERE v.data_vencimento IS NOT NULL
...
AS $$
  SELECT GREATEST(
    0,
    public.receita_previsto_mes(p_ano, p_mes) - public.receita_recebido_mes(p_ano, p_mes)
  )::numeric(15, 2);
$$;

COMMENT ON FUNCTION public.receita_previsto_mes(integer, integer) IS
  'Total faturado (valor_item com vencimento no mês) na cota elegível.';

COMMENT ON FUNCTION public.receita_recebido_mes(integer, integer) IS
  'Recebido no mês apenas de títulos faturados no mesmo mês (cohort).';

COMMENT ON FUNCTION public.receita_inadimplencia_mes_faturado(integer, integer) IS
  'Inadimplência do mês: previsto − recebido (cohort faturado/recebido no mesmo mês).';
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      ROUND(SUM(
        GREATEST(
          0,
          COALESCE(v.valor_item, 0) - COALESCE(
            CASE
              WHEN v.data_pagamento IS NOT NULL
                AND EXTRACT(YEAR FROM v.data_pagamento)::integer = EXTRACT(YEAR FROM v.data_vencimento)::integer
                AND EXTRACT(MONTH FROM v.data_pagamento)::integer = EXTRACT(MONTH FROM v.data_vencimento)::integer
              THEN v.valor_pago_item
              ELSE 0
            END,
            0
          )
        )
      ), 2)::numeric(15, 2) AS valor
    FROM public.receita_itens_inadimplencia_elegiveis v
  ...
  evolucao_calc AS (
    SELECT
      m.mes,
      GREATEST(0, COALESCE(p.previsto, 0) - COALESCE(r.recebido, 0))::numeric(15, 2) AS valor_calc,
  ...
  SELECT
    COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'valor', ec.valor_calc,
        'pct', CASE
          WHEN ec.recebido > 0 THEN ROUND((ec.valor_calc / ec.recebido) * 100, 2)
          ELSE 0
        END,
...
GRANT EXECUTE ON FUNCTION public.receita_previsto_mes(integer, integer) TO anon, authenticated;

-- Recalcula snapshots Jan–Abr/2026 com a nova regra (previsto − recebido cohort).
INSERT INTO public.receita_inadimplencia_fechamento_mensal (ano, mes, valor_total, pct_recebido)
SELECT
  2026,
  gs.m,
  public.receita_inadimplencia_mes_faturado(2026, gs.m),
  public.receita_inadimplencia_pct_mes(2026, gs.m)
FROM generate_series(1, 4) AS gs(m)
