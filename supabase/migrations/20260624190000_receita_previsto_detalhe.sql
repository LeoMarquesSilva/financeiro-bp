-- Drill-down do previsto (data_vencimento + valor_item), mesmo critério de receita_totais_mensais.

CREATE OR REPLACE FUNCTION public.receita_previsto_por_plano(p_ano integer, p_mes integer)
RETURNS TABLE (
  plano_contas text,
  quantidade bigint,
  total numeric
...
COMMENT ON FUNCTION public.receita_previsto_por_plano(integer, integer) IS
  'Breakdown mensal do previsto por plano (cota, vencimento, valor_item, clientes ativos).';

COMMENT ON FUNCTION public.receita_previsto_itens(integer, integer, text) IS
  'Itens com vencimento no mês/plano para drill-down do previsto na UI.';

GRANT EXECUTE ON FUNCTION public.receita_previsto_por_plano(integer, integer) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.receita_previsto_itens(integer, integer, text) TO anon, authenticated;
