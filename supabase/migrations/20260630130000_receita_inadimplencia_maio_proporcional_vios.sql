-- Regra MAIO.xlsx / Planilha4 (VIOS):
-- por cliente no mês = Σ valor_item (vencimento) − Σ valor_pago_item (pagamento).
-- Total mensal = soma líquida por cliente (compensações/antecipados negativos entram).
-- Corrige mai/2026 congelado indevidamente em ~499k.
...
COMMENT ON FUNCTION public.receita_inadimplencia_cliente_mes(integer, integer, boolean) IS
  'Inadimplência mensal VIOS: faturado (vencimento) − recebido (pagamento) por cliente; líquido pode ser negativo.';
...
COMMENT ON FUNCTION public.receita_inadimplencia_grupo_mes(integer, integer, boolean) IS
  'Inadimplência mensal por grupo: soma do líquido VIOS por cliente (inclui compensações negativas).';
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard inadimplência: evolução mensal = soma líquida VIOS (MAIO.xlsx); KPI período = saldos positivos.';

UPDATE public.receita_inadimplencia_fechamento_mensal
SET
  valor_total = 273588.69,
  pct_recebido = ROUND((273588.69 / public.receita_previsto_mes(2026, 5)) * 100, 2),
  congelado_em = now()
WHERE ano = 2026
  AND mes = 5;
