-- KPI acumulado = soma da inadimplência mensal VIOS (mesma regra da evolução).
-- Cada mês: max(0, faturado − recebido) por cliente, com antecipado.
-- Período multi-mês: Σ meses no intervalo. Um único mês = valor da célula mensal.
...
AS $$
  ...
  por_mes AS (
    ...
    CROSS JOIN LATERAL public.receita_inadimplencia_grupo_mes(
  ...
  SELECT
    ...
    MAX(pm.qtd_clientes)::integer AS qtd_clientes
  FROM por_mes pm
  GROUP BY pm.grupo_cliente
  HAVING ROUND(SUM(pm.inadimplencia), 2) > 0
...
COMMENT ON FUNCTION public.receita_inadimplencia_grupos_periodo(integer, integer, integer, boolean) IS
  'Inadimplência acumulada por grupo = Σ inadimplência mensal VIOS no intervalo (mesma regra da evolução).';
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'KPI acumulado = Σ inadimplência mensal VIOS no intervalo (mesma regra da evolução).';
