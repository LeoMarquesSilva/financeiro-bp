-- Performance: entradas novas sem subconsulta correlata por linha (evita timeout).

CREATE OR REPLACE FUNCTION public.receita_recebido_chave_cliente_lookup(
  p_cliente text,
  p_grupo_cliente text DEFAULT NULL
)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  ...
  WHERE (i.tipo IS NULL OR i.tipo = 'RECEBER')
    ...
    AND (
      ...
      OR COALESCE(i.reincidencia_titulo, '') ~ '^1/'
      OR (
...
GRANT EXECUTE ON FUNCTION public.receita_recebido_chave_cliente_lookup(text, text) TO anon, authenticated;
