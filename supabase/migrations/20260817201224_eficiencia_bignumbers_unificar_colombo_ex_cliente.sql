-- Big Numbers: Grupo Colombo pertence ao Grupo Ex-Cliente.
-- Canonicaliza antes da agregação para somar valores e só então montar o TOP 5.
...
BEGIN
  ...
  RETURN (
    ...
    ts AS (
      SELECT
        CASE
          WHEN lower(COALESCE(NULLIF(trim(grupo_cliente), ''), '(sem grupo)'))
            IN ('grupo colombo', 'grupo ex-cliente')
          THEN 'Grupo Ex-Cliente'
          ELSE COALESCE(NULLIF(trim(grupo_cliente), ''), '(sem grupo)')
        END AS grupo,
    ...
    pub AS (
      SELECT
        CASE
          WHEN lower(COALESCE(NULLIF(trim(grupo), ''), '(sem grupo)'))
            IN ('grupo colombo', 'grupo ex-cliente')
          THEN 'Grupo Ex-Cliente'
          ELSE COALESCE(NULLIF(trim(grupo), ''), '(sem grupo)')
        END AS grupo,
...
COMMENT ON FUNCTION public.eficiencia_apresentacao_bignumbers(integer, integer[]) IS
  'Big Numbers comparativo: Grupo Colombo é consolidado em Grupo Ex-Cliente antes dos TOP 5.';
