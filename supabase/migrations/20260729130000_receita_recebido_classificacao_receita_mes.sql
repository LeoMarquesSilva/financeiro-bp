-- Receita do mês: apenas vencimento no mesmo mês/ano do pagamento.
...
COMMENT ON FUNCTION public.receita_recebido_classificacao_mes(integer, integer) IS
  'Itens recebidos no mês: novos_contratos (1º pagamento), inadimplencia (venc. anterior), receita_mes (venc. no mês ou demais).';
