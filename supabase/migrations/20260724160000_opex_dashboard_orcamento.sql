-- OPEX dashboard: previsto principal = orçamento congelado; previsto_vios = VIOS operacional.
...
COMMENT ON FUNCTION public.opex_dashboard(integer, integer[]) IS
  'Dashboard OPEX: previsto = orçamento congelado (se importado) ou VIOS; previsto_vios sempre operacional.';
...
DROP FUNCTION IF EXISTS public.opex_mes_itens(integer, integer);
...
AS $$
  ...
  FROM (
    SELECT
      ...
      i.data_pagamento,
      round(CASE
        WHEN public.opex_tem_orcamento(p_ano) THEN coalesce((
          SELECT sum(l.valor)
          FROM public.opex_orcamento_linha l
          WHERE l.ano = p_ano AND l.mes = p_mes
