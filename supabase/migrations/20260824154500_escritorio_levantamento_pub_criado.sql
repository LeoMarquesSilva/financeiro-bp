-- Levantamento Escritório: período de publicações pela coluna Criado (lista VIOS).

DROP FUNCTION IF EXISTS public.escritorio_levantamento_pub_data_ref(timestamptz, date);

CREATE OR REPLACE FUNCTION public.escritorio_levantamento_pub_data_ref(p_criado timestamptz)
RETURNS date
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT (p_criado AT TIME ZONE 'America/Sao_Paulo')::date;
$$;

COMMENT ON FUNCTION public.escritorio_levantamento_pub_data_ref(timestamptz) IS
  'Data de referência para filtro de período no levantamento: campo Criado da lista de publicações (timezone SP).';
      ...
      FROM (
        ...
        FROM (
          ...
          WHERE p.criado IS NOT NULL
            AND public.escritorio_levantamento_pub_data_ref(p.criado)
...
COMMENT ON FUNCTION public.escritorio_levantamento_resumo_v2(date, date, text[], text) IS
  'KPIs levantamento Escritório. Publicações: período pelo campo Criado (lista VIOS).';

COMMENT ON FUNCTION public.escritorio_levantamento_racional_v2(text, date, date, text[], text, text, integer) IS
  'Racional levantamento Escritório — publicações filtradas por Criado (timezone America/Sao_Paulo).';

GRANT EXECUTE ON FUNCTION public.escritorio_levantamento_pub_data_ref(timestamptz) TO anon, authenticated;
