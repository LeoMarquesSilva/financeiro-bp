-- KPI e top5 do período: saldo em aberto no último dia do mês fim (mesma base da lista de clientes).
-- A evolução mensal permanece mês a mês (histórico).
...
BEGIN
  ...
  SELECT COALESCE(SUM(c.valor), 0)::numeric(15, 2)
  INTO v_valor_periodo
  FROM public.receita_inadimplencia_clientes_periodo(p_ano, v_mes_inicio, v_mes_fim) c;

  SELECT COALESCE(jsonb_agg(row_to_json(t)::jsonb ORDER BY t.valor DESC), '[]'::jsonb)
  INTO v_top5
  FROM (
    SELECT c.cliente, c.valor
    FROM public.receita_inadimplencia_clientes_periodo(p_ano, v_mes_inicio, v_mes_fim) c
    WHERE c.valor > 0
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard inadimplência: KPI/top5 = saldo no fim do período; evolução = fechamento mensal.';
