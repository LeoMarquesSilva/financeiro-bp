-- KPI acumulado usa soma de valor_calculado (VIOS ao vivo), não snapshots congelados.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(ROUND(SUM(ec.valor_calc), 2), 0)::numeric(15, 2)
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard inadimplência: KPI = soma valor_calculado ao vivo; snapshots congelados só como referência.';
