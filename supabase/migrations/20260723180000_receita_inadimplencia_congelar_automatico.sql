-- Congela automaticamente meses já encerrados na virada do mês.
-- Usa a seleção de grupos salva ou, se ausente, todos os grupos inadimplentes (mesma regra do sheet).

CREATE OR REPLACE FUNCTION public.receita_inadimplencia_congelar_meses_encerrados(p_ano integer)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FOR v_mes IN 1..v_mes_fim LOOP
    ...
    WHERE s.ano = p_ano AND s.mes = v_mes;

    IF v_grupos IS NULL OR cardinality(v_grupos) = 0 THEN
      SELECT COALESCE(array_agg(g.grupo_cliente ORDER BY g.grupo_cliente), '{}'::text[])
      INTO v_grupos
      FROM public.receita_inadimplencia_grupo_mes(p_ano, v_mes, true) g
    ...
    INTO v_valor
    FROM public.receita_inadimplencia_grupo_mes(p_ano, v_mes, true) g
    WHERE g.grupo_cliente = ANY(v_grupos);

    v_previsto := public.receita_previsto_mes(p_ano, v_mes)::numeric(15, 2);

    v_pct := CASE
      WHEN v_previsto > 0 THEN ROUND((v_valor / v_previsto) * 100, 2)
      ELSE 0
    END;

    PERFORM public.receita_inadimplencia_congelar_mes(p_ano, v_mes, v_valor, v_pct);
  END LOOP;
END;
$$;

COMMENT ON FUNCTION public.receita_inadimplencia_congelar_meses_encerrados(integer) IS
  'Congela meses encerrados (mes < mês corrente) com seleção de grupos salva ou padrão inadimplentes; não sobrescreve fechamentos existentes.';

GRANT EXECUTE ON FUNCTION public.receita_inadimplencia_congelar_meses_encerrados(integer) TO anon, authenticated;
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'KPI acumulado = saldo líquido do período (grupos_periodo); evolução = valores mensais (congelados ou ao vivo). Congela meses encerrados automaticamente.';
