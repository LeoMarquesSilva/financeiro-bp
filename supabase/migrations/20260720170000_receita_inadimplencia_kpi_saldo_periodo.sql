...
-- KPI acumulado = saldo líquido do período (grupos_periodo), não Σ evolução mensal.
-- Evolução mantém snapshots congelados; o card Resultado R$ reflete inadimplência real do intervalo.
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'KPI acumulado = saldo líquido do período (grupos_periodo); evolução = valores mensais (congelados ou ao vivo).';
