-- Kanban inadimplência: valor em aberto alinhado à Receita (saldo líquido por grupo no ano corrente).
-- Evita subestimar grupos como CDA quando o VIOS só tem parcelas spot abertas.
...
WITH bounds AS (
  SELECT
    ...
    1 AS mes_inicio,
    LEAST(12, GREATEST(1, EXTRACT(MONTH FROM CURRENT_DATE)::integer)) AS mes_fim
),
receita_grupo_saldo AS (
  SELECT
    g.grupo_cliente,
    g.valor AS valor_receita_grupo
  FROM bounds b
...
base AS (
  ...
  FROM cliente_base cb
  LEFT JOIN receita_grupo_saldo rg ON rg.grupo_cliente = cb.grupo_cliente
...
COMMENT ON VIEW public.clients_inadimplencia_list IS
  'Lista inadimplência: valor por saldo Receita (faturado − recebido) do grupo no ano corrente; fallback parcelas VIOS vencidas.';
