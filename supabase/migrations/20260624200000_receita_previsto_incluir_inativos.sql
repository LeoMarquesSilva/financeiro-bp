-- Drill-down do previsto: opção de incluir clientes inativos (padrão = só ativos, como KPI).
...
COMMENT ON FUNCTION public.receita_previsto_por_plano(integer, integer, boolean) IS
  'Breakdown mensal do previsto por plano. p_incluir_inativos=false (padrão) alinha ao KPI.';

COMMENT ON FUNCTION public.receita_previsto_itens(integer, integer, text, boolean) IS
  'Itens com vencimento no mês/plano. p_incluir_inativos opcional para conferência.';
