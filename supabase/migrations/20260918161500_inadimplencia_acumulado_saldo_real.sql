-- Acumulado = inadimplência real atual (faturado − pago dos títulos vencidos).
-- Não é a soma do fechamento de 2025 com o gerado de 2026: pagamento do estoque
-- antigo em 2026 reduz o acumulado, sem apagar o Saldo 2025 nem o gerado do ano.
...
COMMENT ON FUNCTION public.inadimplencia_evolucao_saldo_devedor(integer, integer) IS
  'Saldo anterior = fechamento do ano anterior (sem baixa posterior). Gerado = títulos do ano ainda em aberto. Acumulado = inadimplência real atual (faturado − pago), não a soma das duas colunas.';
