-- Títulos por plano / departamento: expor valor_orcamento (orçamento congelado).
...
DROP FUNCTION IF EXISTS public.opex_departamento_titulos(integer, text, text, text, integer[], boolean);
    ...
    SELECT
      ...
      round(coalesce((
        ...
        WHERE l.ano = p_ano
          ...
          AND l.grupo_conta = coalesce(nullif(trim(i.grupo_conta), ''), 'Sem grupo')
          AND l.plano_contas = coalesce(nullif(trim(i.plano_contas), ''), 'Sem plano')
          AND l.departamento = coalesce(nullif(trim(i.departamento), ''), 'Sem departamento')
          AND (
            l.titulo_ref = coalesce(nullif(trim(i.nro_titulo), ''), '—')
            OR l.titulo_ref = coalesce(nullif(trim(i.descricao), ''), 'Sem descrição')
          )
