-- Inclui o mês corrente no filtro de inadimplência (antes só meses encerrados).
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'Dashboard inadimplência. mes_max_disponivel inclui o mês corrente no ano em curso.';
