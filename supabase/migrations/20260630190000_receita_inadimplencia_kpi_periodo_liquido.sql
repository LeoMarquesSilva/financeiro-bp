-- KPI acumulado = saldo líquido do PERÍODO (faturado vencimento − recebido pagamento),
-- com compensações (saldos negativos por cliente reduzem o total).
-- NÃO somar inadimplência mensal (infla ~2×). Evolução mensal permanece separada.
...
AS $$
  SELECT
    n.grupo_cliente,
    ROUND(SUM(n.valor_liquido), 2)::numeric(15, 2) AS valor,
  ...
  HAVING ROUND(SUM(n.valor_liquido), 2) <> 0
...
COMMENT ON FUNCTION public.receita_inadimplencia_grupos_periodo(integer, integer, integer, boolean) IS
  'Saldo líquido do período por grupo (Σ cliente faturado − Σ recebido), inclui compensações negativas.';
...
BEGIN
  ...
  -- KPI: saldo líquido do intervalo (compensações entram), inclui inativos
  SELECT COALESCE(ROUND(SUM(n.valor_liquido), 2), 0)::numeric(15, 2)
...
COMMENT ON FUNCTION public.receita_inadimplencia_dashboard(integer, integer, integer) IS
  'KPI acumulado = saldo líquido do período com compensações; evolução = valores mensais separados.';
