-- Módulo OPEX: despesas operacionais (tipo PAGAR) com previsto x realizado e projeção.

CREATE OR REPLACE FUNCTION public.financeiro_titulo_eh_pagar(p_tipo text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT upper(trim(coalesce(p_tipo, ''))) = 'PAGAR';
$$;

COMMENT ON FUNCTION public.financeiro_titulo_eh_pagar(text) IS
  'True para títulos a pagar (OPEX e demais saídas).';

GRANT EXECUTE ON FUNCTION public.financeiro_titulo_eh_pagar(text) TO anon, authenticated;

CREATE OR REPLACE FUNCTION public.opex_norm_grupo(p_grupo text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION public.opex_grupo_excluido(p_grupo text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT public.opex_norm_grupo(p_grupo) = ANY(ARRAY[
...
CREATE OR REPLACE FUNCTION public.opex_grupo_fixo(p_grupo text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT public.opex_norm_grupo(p_grupo) = ANY(ARRAY[
    ...
    'MATERIAL DE CONSUMO, LIMPEZA E SEGURANCA',
    'DESPESAS DE T.I.',
    'DESPESAS DE COPA E COZINHA'
  ]);
$$;

CREATE OR REPLACE FUNCTION public.opex_item_elegivel(i public.financeiro_parcelas_itens)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT public.financeiro_titulo_eh_pagar(i.tipo)
    AND NOT public.opex_grupo_excluido(i.grupo_conta);
$$;

CREATE OR REPLACE FUNCTION public.opex_valor_item(i public.financeiro_parcelas_itens)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT abs(coalesce(i.valor_item, i.valor_fluxo_item, 0));
$$;

CREATE OR REPLACE FUNCTION public.opex_valor_pago(i public.financeiro_parcelas_itens)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT abs(coalesce(i.valor_pago_item, 0));
$$;

CREATE INDEX IF NOT EXISTS idx_financeiro_parcelas_itens_pagar_venc
  ON public.financeiro_parcelas_itens (data_vencimento)
  WHERE upper(trim(tipo)) = 'PAGAR';

CREATE INDEX IF NOT EXISTS idx_financeiro_parcelas_itens_pagar_pag
  ON public.financeiro_parcelas_itens (data_pagamento)
  WHERE upper(trim(tipo)) = 'PAGAR';

CREATE OR REPLACE FUNCTION public.opex_dashboard(p_ano integer)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  grupos AS (
    SELECT
      ...
      round(sum(public.opex_valor_pago(i)) FILTER (
        WHERE i.data_pagamento IS NOT NULL
          AND extract(year FROM i.data_pagamento)::integer = p_ano
          AND (v_mes_atual = 0 OR extract(month FROM i.data_pagamento)::integer <= v_mes_atual)
      ...
      round(sum(public.opex_valor_item(i)) FILTER (
        WHERE i.data_vencimento IS NOT NULL
          AND extract(year FROM i.data_vencimento)::integer = p_ano
          AND extract(month FROM i.data_vencimento)::integer > v_mes_atual
  ...
  totais AS (
    SELECT
      round(coalesce(sum(realizado) FILTER (WHERE mes <= v_mes_atual), 0)::numeric, 2) AS realizado_ytd,
      round(coalesce(sum(previsto) FILTER (WHERE mes <= v_mes_atual), 0)::numeric, 2) AS previsto_ytd,
      ...
      SELECT coalesce(jsonb_agg(
        jsonb_build_object(
          ...
          'projetado_ano', round((g.realizado_ytd + g.previsto_restante
            + CASE WHEN g.fixo AND v_mes_atual > 0
              THEN v_media_fixas * (12 - v_mes_atual) ELSE 0 END)::numeric, 2)
      ...
      FROM (
        SELECT
          ...
          round(sum(public.opex_valor_pago(i)) FILTER (
            WHERE i.data_pagamento IS NOT NULL
              AND extract(year FROM i.data_pagamento)::int = p_ano
              AND (v_mes_atual = 0 OR extract(month FROM i.data_pagamento)::int <= v_mes_atual)
          )::numeric, 2) AS realizado_ytd,
          round(sum(public.opex_valor_item(i)) FILTER (
            WHERE i.data_vencimento IS NOT NULL
              AND extract(year FROM i.data_vencimento)::int = p_ano
...
COMMENT ON FUNCTION public.opex_dashboard(integer) IS
  'Dashboard OPEX: evolução mensal previsto x realizado, grupos de conta e projeção de despesas fixas.';

GRANT EXECUTE ON FUNCTION public.opex_dashboard(integer) TO anon, authenticated;

CREATE OR REPLACE FUNCTION public.opex_planos_grupo(p_ano integer, p_grupo text)
RETURNS TABLE (
  plano_contas text,
...
AS $$
  ...
  SELECT
    ...
    round(sum(public.opex_valor_pago(i)) FILTER (
      WHERE i.data_pagamento IS NOT NULL
        AND extract(year FROM i.data_pagamento)::int = p_ano
        AND (ctx.mes_atual = 0 OR extract(month FROM i.data_pagamento)::int <= ctx.mes_atual)
    )::numeric, 2) AS realizado_ytd,
    round(sum(public.opex_valor_item(i)) FILTER (
      WHERE i.data_vencimento IS NOT NULL
        AND extract(year FROM i.data_vencimento)::int = p_ano
...
GRANT EXECUTE ON FUNCTION public.opex_planos_grupo(integer, text) TO anon, authenticated;
