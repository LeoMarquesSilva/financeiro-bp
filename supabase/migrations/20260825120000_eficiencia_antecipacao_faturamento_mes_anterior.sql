-- Antecipação Honorários: faturamento com prazo estimado no mês anterior,
-- concluído no mês seguinte, conta como "dentro do prazo" (ex.: mai/26 → jun/26).
...
AS $$
  WITH base AS (
    SELECT
      ...
      CASE
        WHEN data_limite IS NOT NULL AND data_conclusao <= data_limite THEN 'dentro'
        WHEN date_trunc('month', data_para_conclusao)::date
          < date_trunc('month', data_conclusao)::date THEN 'dentro'
        ELSE 'fora'
...
COMMENT ON FUNCTION public.eficiencia_ops_legais_antecipacao_mensal(integer) IS
  'BI Ops Legais / Antecipação Honorários: % dentro do prazo por mês de conclusão. '
  'Inclui faturamentos com prazo estimado no mês anterior concluídos no mês seguinte.';
