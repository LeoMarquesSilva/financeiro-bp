-- SharePoint: "Catharina Silva" · turnover: "CATHARINA CARDOSO DA SILVA"
...
AS $$
  SELECT CASE base
    ...
    WHEN 'CATHARINA SILVA'
      THEN 'CATHARINA CARDOSO DA SILVA'
    ELSE base
  END
  FROM (
    SELECT NULLIF(
...
COMMENT ON FUNCTION public.eficiencia_nome_chave(text) IS
  'Chave de match pessoa: strip Ex Func + UPPER(unaccent(trim)) + aliases AD/SharePoint → nome turnover.';
