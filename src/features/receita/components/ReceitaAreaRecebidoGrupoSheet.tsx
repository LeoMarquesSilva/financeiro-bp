...
import { exportAreaRecebidoGrupoExcel } from '../utils/receitaAreaRecebidoExport'
...
export function ReceitaAreaRecebidoGrupoSheet({
  open,
  onOpenChange,
  ano,
  mes,
  ...
  useEffect(() => {
    ...
    Promise.all([
      receitaService.fetchRecebidoItensPorArea(ano, mes, areaKey),
      ...
      .catch((e) => {
        if (!cancelled) {
          setErrorItens(e instanceof Error ? e.message : 'Erro ao carregar recebimentos.')
          setItens([])
        }
      })
      .finally(() => {
  ...
  const handleExportar = async () => {
    ...
    setExportando(true)
    try {
      await exportAreaRecebidoGrupoExcel(gruposAgg, itens, clienteGrupoMap, {
        ano,
        mes,
        mesLabel,
        areaKey,
            ...
            <>
              <div className="flex items-start justify-between gap-3">
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="-ml-2 h-8 gap-1.5 px-2 text-sky-100 hover:bg-white/10 hover:text-white"
            ...
            {view === 'grupos' && !loadingItens && !errorItens && gruposFiltrados.length === 0 && (
              <p className="py-10 text-center text-sm text-slate-500">
                {buscaDebounced
                  ...
                  : 'Nenhum recebimento nesta área no mês.'}
              </p>
            )}
          ...
          {view === 'grupos' && !loadingItens && gruposFiltrados.length > 0 && (
            <div className="shrink-0 border-t border-slate-200 bg-slate-50 px-6 py-3">
              <div className="flex items-center justify-between text-sm font-semibold text-slate-800">
                ...
                <span className="tabular-nums text-sky-800">{formatCurrency(totalGruposFiltrados)}</span>
              </div>
            </div>
          )}
          ...
          {view === 'titulos' && !loadingItens && titulosFiltrados.length > 0 && (
            <div className="shrink-0 border-t border-slate-200 bg-slate-50 px-6 py-3">
              <div className="flex items-center justify-between text-sm font-semibold text-slate-800">
                ...
                <span className="tabular-nums text-sky-800">{formatCurrency(totalTitulosFiltrados)}</span>
              </div>
            </div>
          )}
        </div>
